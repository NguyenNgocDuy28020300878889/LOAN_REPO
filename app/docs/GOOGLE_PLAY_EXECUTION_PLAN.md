...
| P1  | **Đã sửa vòng đời lời mời ở local**          | Migration `20260923120000_invite_lifecycle.sql` buộc lời mời gắn email tuân thủ hết hạn/thu hồi, cho phép chủ khoản vay cấp lại link, ẩn lời mời không còn hiệu lực và vô hiệu push đang chờ. Regression mới chứng minh không thể dùng `loan_id` để bỏ qua thu hồi/hết hạn.                                                                                                                    | Chỉ dry-run và áp dụng lên STAGING sau khi commit ứng viên có cả hai job CI xanh.                                                                          |
...
| P1  | **CI đã xanh**                               | Commit `316a7554742113e35f502e01595882d071071385` có cả hai job `quality` và `database` thành công tại [run `35837096936`](https://github.com/NguyenNgocDuy28020300878889/LOAN_REPO/actions/runs/35837096936). Job database đạt replay migration, SQL regression, concurrency, auth/recovery, browser export và hành trình hai tài khoản/outsider.                                             | Giữ branch protection yêu cầu cả hai job; xử lý ngay nếu regression mới làm một trong hai job thất bại.                                                    |
...
- Migration history STAGING hiện khớp toàn bộ **21 migration files**. Contract mới đã được dry-run, áp dụng và kiểm tra strict 16 assertions trong transaction rollback; không giữ fixture trên cloud.
...
**Cập nhật sau sửa contract:** lỗi P0 và hai khoảng trống P1 liên quan đã đóng trên local, GitHub CI và STAGING. Bước 2 tiếp tục với nghiệm thu Android hai thiết bị và các trạng thái native/network chưa có bằng chứng.
...
- Commit `316a7554742113e35f502e01595882d071071385` đã qua cả hai job GitHub CI. STAGING `kircmwdkcdcozckrwfid` đã nhận migration thứ 21 sau khi dry-run xác nhận đúng một file pending; regression contract strict 16 assertions đạt và rollback toàn bộ fixture.
...
Bước 2 chưa hoàn tất trên native: máy hiện không có thiết bị ADB hoặc AVD, nên chưa có bằng chứng hai thiết bị Android cho Google callback, chấp nhận/từ chối lời mời, bàn phím, cold/warm link, foreground/reconnect và push. Migration thứ 22 cũng chưa được đưa lên STAGING trước khi CI ứng viên xanh.
