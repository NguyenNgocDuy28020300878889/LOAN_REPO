-- Qualify pgcrypto calls: Supabase installs the extension in extensions.
-- Preserve the restricted search_path and immutable migration history.
do $$ begin
  if not exists (select 1 from pg_extension e join pg_namespace n on n.oid = e.extnamespace where e.extname = 'pgcrypto' and n.nspname = 'extensions') then
    raise exception 'PGCRYPTO_SCHEMA_REVIEW_REQUIRED';
  end if;
end; $$;
...
revoke execute on function public.get_loan_invite_preview(text) from public, anon;
grant execute on function public.get_loan_invite_preview(text) to authenticated;
