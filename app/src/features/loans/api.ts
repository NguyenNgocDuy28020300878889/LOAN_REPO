...
export async function createLoan(input: CreateLoanInput) {
  ...
  const { data, error } = await getSupabaseClient().rpc('create_loan', {
  ...
  return data as { loan_id: string; invite_token: string; status: 'PENDING' };
}

export async function acceptLoanInvite(inviteToken: string, idempotencyKey: string) {
  const { data, error } = await getSupabaseClient().rpc('accept_loan_invite', {
...
export async function declineLoanInvite(inviteToken: string, idempotencyKey: string) {
  const { data, error } = await getSupabaseClient().rpc('decline_loan_invite', {
    invite_token_input: inviteToken,
    idempotency_key_input: idempotencyKey,
  });
  if (error) throw error;
...
export async function getLoanInvitePreview(inviteToken: string) {
  const { data, error } = await getSupabaseClient().rpc('get_loan_invite_preview', {
    invite_token_input: inviteToken,
  });
  if (error) throw error;
...
export async function getMyLoans() {
  const { data, error } = await getSupabaseClient().rpc('get_my_loans');
  if (error) throw error;
  return data as LoanSummary[];
}

export async function getLoanRoom(loanId: string) {
  const { data, error } = await getSupabaseClient().rpc('get_loan_room', {
...
export async function getLoanRepayments(loanId: string) {
  const { data, error } = await getSupabaseClient().rpc('get_loan_repayments', {
    loan_id_input: loanId,
  });
  if (error) throw error;
...
export async function submitRepayment(input: {
  ...
  const { data, error } = await getSupabaseClient().rpc('submit_repayment', {
